...
if (process.platform == "win32") {
    SQL_interogation = "SELECT AVG(longevitate) AS \"Medie_viata\"," +
        ...
        "(SELECT COUNT(*) FROM animals WHERE TRIM(cardModDeHranire) = \"parazitar\") AS \"Numar_animale_parazitare\"," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Mamifere\") AS \"Greutate_medie_mamifere\" ," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Pasari\") AS \"Greutate_medie_pasari\" ," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Insecte\") AS \"Greutate_medie_insecte\" ," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Reptile\") AS \"Greutate_medie_reptile\" ," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Dinozauri\") AS \"Greutate_medie_dinozauri\" ," +
        "(SELECT AVG(greutate) FROM animals WHERE TRIM(cardCategorie) = \"Pesti\") AS \"Greutate_medie_pesti\" ," +
        "(SELECT AVG(inaltime) FROM animals WHERE TRIM(cardCategorie) = \"Mamifere\") AS \"Inaltime_medie_mamifere\" ," +
        "(SELECT AVG(inaltime) FROM animals WHERE TRIM(cardCategorie) = \"Pasari\") AS \"Inaltime_medie_pasari\" ," +
        "(SELECT AVG(inaltime) FROM animals WHERE TRIM(cardCategorie) = \"Dinozauri\") AS \"Inaltime_medie_dinozauri\" ," +
        "(SELECT AVG(inaltime) FROM animals WHERE TRIM(cardCategorie) = \"Reptile\") AS \"Inaltime_medie_reptile\" ," +
...
connection.query({ sql: SQL_interogation, timeout: 1000 }, function(error, results, fields) {
    ...
    chart.setConfig({
        ...
        data: {
            labels: ['Mamifere', 'Pasari', 'Reptile', 'Dinozauri', 'Insecte', 'Pesti'],
            datasets: [{
                label: 'Greutatea medie a animalelor( kilograme )',
                data: [results[0].Greutate_medie_mamifere, results[0].Greutate_medie_pasari, results[0].Greutate_medie_reptile, results[0].Greutate_medie_dinozauri, results[0].Greutate_medie_insecte, results[0].Greutate_medie_pesti]
            }]
        },
    });

    chart.setBackgroundColor("transparent");
    chart.toFile('chart5.png');
    ...
    chart.setConfig({
        type: 'bar',
        data: {
            labels: ['Mamifere', 'Pasari', 'Reptile', 'Dinozauri'],
            datasets: [{
                label: 'Inaltime medie a animalelor( metri )',
                data: [results[0].Inaltime_medie_mamifere, results[0].Inaltime_medie_pasari, results[0].Inaltime_medie_reptile, results[0].Inaltime_medie_dinozauri]
            }]
        },
    });

    chart.setBackgroundColor("transparent");
    chart.toFile('chart6.png');
    connection.destroy();
